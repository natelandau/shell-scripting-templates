...
#  bfl::verify_arg_count "$#" 1 2  || { bfl::writelog_fail "${FUNCNAME[0]} arguments count $# ∉ [1, 2]";  return ${BFL_ErrCode_Not_verified_args_count}; }  # Verify argument count.
  bfl::verify_dependencies "proxychains4" || { bfl::writelog_fail "${FUNCNAME[0]}: dependency 'proxychains4' not found"; return ${BFL_ErrCode_Not_verified_dependency}; }  # Verify dependencies.
