...
#  $(bfl::is_Terminal) || { bfl::writelog_fail "${FUNCNAME[0]}: no terminal found"; return 1; }  # Do nothing if the output is not a terminal
  [[ ${_BFL_HAS_SHOPT} -eq 1 ]]  || { bfl::writelog_fail "${FUNCNAME[0]}: dependency 'shopt' not found"; return ${BFL_ErrCode_Not_verified_dependency}; }  # Verify dependencies.
  [[ ${_BFL_HAS_TRAP} -eq 1 ]]   || { bfl::writelog_fail "${FUNCNAME[0]}: dependency 'trap' not found";  return ${BFL_ErrCode_Not_verified_dependency}; }  # Verify dependencies.
