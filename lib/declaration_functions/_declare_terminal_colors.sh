...
#  bfl::verify_dependencies "tput" || { bfl::writelog_fail "${FUNCNAME[0]}: dependency tput not found"; return $BFL_ErrCode_Not_verified_dependency; }  # Verify dependencies.
  ...
  if $has_tput; then  # If tput is present, prefer it over the escape sequence based formatting
      ...
      # Enable additional formatting for 256 color terminals (on 8 color terminals the formatting likely is implemented as a brighter color rather than a different font)
      $use256 && declare -gr FMT_BOLD="$(printf '\033[01m')"
      $use256 && declare -gr FMT_UNDERLINE="$(printf '\033[04m')"
  fi
  ...
  if ! $bEnabled; then
      if $has_tput; then
          ...
          declare -gr CLR_INFORM="$(tput setaf 2)"  # Green
          ...
          $use256 && declare -gr CLR_INFORM="$(printf '\033[38;5;2m')"   || declare -gr CLR_INFORM="$(printf '\033[32m')"
